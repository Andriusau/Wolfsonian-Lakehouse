  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const dataQuery = `
        ...
        FROM catalog 
        WHERE list_contains(string_split(field_place_published, '|'), '${escapedCreator}')
        ORDER BY has_image DESC, title ASC 
      `;
      
      const countQuery = `SELECT count(*) as total FROM catalog WHERE list_contains(string_split(field_place_published, '|'), '${escapedCreator}')`;
            ...
            <div className="text-center py-32 border border-white/20 bg-mca-black space-y-4 max-w-md mx-auto p-8">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-500 font-sans leading-relaxed">
                  The query returned zero rows for this place.
                </p>
              </div>
            </div>
          )}
                        ...
                        .map(([key, val], i) => {
                          ...
                          return (
                            <div key={i} className="flex flex-col space-y-2 group">
                              <h2 className="text-[10px] md:text-xs text-mca-cyan font-bold tracking-[0.2em] mb-4">PLACE DOSSIER</h2>
