...
|---|---|---|---|
| **Alma** | Ex Libris Library Management | 54,941 | Binary MARC (`.mrc`) parsing via PyMARC & Physical Item (`.csv`) mapping |
| **Proficio** | Museum Collection Database | 61,049 | Kerberos-authenticated SQL Server via ODBC |
| **Islandora** | Public Digital Archive | 268,795 | Paginated REST API with concurrent fetching |
| **Unified Gold Catalog** | Merged output | 115,990 | Alma + Proficio aligned and concatenated |
| **Normalized Gold Catalog** | Analytics-ready output | 115,990 | Harmonized genres, dates, creators & titles |
| **Records with Images** | Gold Catalog Filter | 55,687 | Distinct records possessing at least one valid image (a single record can have up to 1,000+ images for books/albums) |
| **Digital Images** | NFS Mounted Share | 342,931 | Parallel ingestion and JPEG compression |
...
* **Creator Relator Standardization & Metabase Protection:** Standardizes MARC and Proficio agent roles into `relators:{code}:person:{name}` during Silver processing, and normalizes them in Gold using a non-breaking dual-column strategy:
  ...
  * `creator_roles`: Stores distinct roles present on each record (`Designer | Maker`) for high-speed faceted search queries.
...
* **Location Search:** Searches include object `location` and `storage_location` fields for box and shelf lookups such as `OB270`, `TX25`, and `SU41_A`. Exact location matches receive an 8-point ranking boost, while partial matches receive 2 points.
